...
class LostFoundItem(models.Model):
    ...
    # Found info (optional; used for map pins)
    found_lat = models.FloatField(null=True, blank=True)
    found_lng = models.FloatField(null=True, blank=True)
    found_at = models.DateTimeField(null=True, blank=True)

    # Who reported it (either a student or a teacher)
    ...
    reporter_teacher = models.ForeignKey(
        ...
        related_name="reported_lostfound_items"
    )
    ...
    correct_answer = models.CharField(
        ...
        help_text="Optional exact answer; if provided, matching claims auto-approve."
    )
    ...
    def has_coordinates(self):
        return self.found_lat is not None and self.found_lng is not None
...
class Claim(models.Model):
    ...
    item = models.ForeignKey(
        LostFoundItem, on_delete=models.CASCADE, related_name="claims"
    )
    claimant_student = models.ForeignKey(
        Student, null=True, blank=True, on_delete=models.SET_NULL
    )
    claimant_teacher = models.ForeignKey(
        Teacher, null=True, blank=True, on_delete=models.SET_NULL,
    ...
    def __str__(self):
        who = self.claimant_student or self.claimant_teacher
        return f"Claim: {self.item.title} by {getattr(who, 'name', 'Unknown')} [{self.status}]"
